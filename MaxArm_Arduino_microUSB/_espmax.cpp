...
float* inverse(float pos[3], float* ang){
    ...
    float r = sqrt(x*x + y*y) - L1 - L4; //Rotation radius (旋转半径)
    z = z - L0;
    ...
    //Use cosine theorem to solve joint angles (余弦定理求各关节夹角)
    ...
    //Adjust the angle offset as needed. Here, the chassis rotation start is set to -120°, and the coordinate system is rotated. (根据需要将角度进行一下偏转，这里将底盘旋转的起始处定为-120度，将坐标系旋转)
