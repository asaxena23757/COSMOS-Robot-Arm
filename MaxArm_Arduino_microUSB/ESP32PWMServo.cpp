    ...
    if(0 < id & id < 3){
        ...
        if(stat){
            ...
            for(int count=0; count<int(degree); count++){            
                pulse = int(pulse + d);
                if(id == 1)servo1.writeMicroseconds(pulse);    // Output PWM (输出PWM)
                else if(id == 2)servo2.writeMicroseconds(pulse); // Output PWM (输出PWM)
                delay(20);
            }
            init_pulse[id-1] = pul;
        }  
        else{
            if(id == 1)servo1.writeMicroseconds(pul); // Output PWM (输出PWM)
            else if(id == 2)servo2.writeMicroseconds(pul); // Output PWM (输出PWM)
            init_pulse[id-1] = pul;
            stat = 1;
        }
    }
